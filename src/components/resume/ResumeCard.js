...
const ResumeCard = ({title,subTitle,result,des}) => {
  return (
    <div className='w-full h-full group flex'>
      ...
      rounded-lg px-4 lgl:px-20 flex flex-col justify-center gap-6 lgl:gap-10 shadow-shadowOne'>
        <div className='flex flex-col lgl:flex-row justify-between gap-4 lgl:gap-0 lgl:items-center'>
          <div>
            <h3 className='text-xl md:text-3xl font-semibold group-hover:text-white duration-300'>
              {title}
            </h3>
            <p className='text-sm mt-2 text-designColor group-hover:text-designColor duration-300'>
              {subTitle}
            </p>
          </div>
